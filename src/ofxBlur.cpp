...
string generateBlurSource(int radius, float shape) {
  ...
  src << "void main(void) {\n";
  src << "\tfragColor = " << coefficients[0] << " * texture2D(source, texCoordVarying);\n";
  for(int i = 1; i < coefficients.size(); i++) {
    int curOffset = i - 1;
    src << "\tfragColor += " << coefficients[i] << " * \n";
    src << "\t\t(texture2D(source, texCoordVarying - (direction * " << offsets[i - 1] << ")) + \n";
    src << "\t\ttexture2D(source, texCoordVarying + (direction * " << offsets[i - 1] << ")));\n";
  }
  src << "}\n";
...
string generateCombineSource(int passes, float downsample) {
  ...
  src << "void main(void) {\n";
  src << "vec2 tc = texCoordVarying;\n";
  for(int i = 0; i < passes; i++) {
    src << "\tfragColor " << (i == 0 ? "=" : "+=");
    src << " texture2D(" << combineNames[i] << ", tc);";
    src << (i + 1 != passes ? " tc *= scaleFactor;" : "");
    src << "\n";
  }
  src << "\tfragColor *= brightness / " << passes << ".;\n";
  src << "}\n";
  return src.str();
}
...
void ofxBlur::setup(int width, int height, int radius, float shape, int passes, float downsample) {
  ...
  string defaultVertexShader = R"(#version 150
  ...
  blurShader.setupShaderFromSource(GL_VERTEX_SHADER, defaultVertexShader);
  ...
  if(passes > 1) {
    ...
    combineShader.setupShaderFromSource(GL_VERTEX_SHADER, defaultVertexShader);
...
void ofxBlur::end() {
  ...
  for(int i = 0; i < ping.size(); i++) {
    ...
    blurShader.setUniform2f("direction", xDirection.x / base.getWidth(), xDirection.y / base.getHeight());
    curPing.draw(0, 0, w, h);
    blurShader.end();
    curPong.end();
    ...
    blurShader.setUniform2f("direction", yDirection.x / base.getWidth(), yDirection.y / base.getHeight());
    curPong.draw(0, 0, w, h);
    blurShader.end();
    curPing.end();
  }
...
void ofxBlur::draw(ofRectangle rect) {
    base.draw(rect.x, rect.y, rect.width, rect.height);
}
